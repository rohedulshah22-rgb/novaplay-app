...
If no subtitle cue is available and no speech backend is configured, NovaPlay leaves the video viewport clear and shows the one-time SnackBar message `AI Live Subtitles currently requires cloud relay setup`. It does not show a blocking HUD error on every polling cycle. Protected `content://` media sources use the non-blocking audio-backend path because FFmpegKit cannot directly read those protected URIs. The caption overlay is bottom-aligned, high contrast, shadowed, and labeled with the selected target language. The service is owned by `PlayerScreen`, and its HTTP client, temporary subtitle files, and polling timer are closed when the player route is disposed.
